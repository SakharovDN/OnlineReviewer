...
export class TokenService {
    ...
    async VerifyAccessToken(request): Promise<boolean> {
        try{
            ...
            const user = this.jwtService.verify(token, { secret: 'process.env.ACCESS_TOKEN_SECRET' });
            request.user = user;
            return true;
        }
        catch(e){
    ...
    async VerifyRefreshToken(token: string, request: any): Promise<boolean> {
        try{
            const user = this.jwtService.verify(token, { secret: 'process.env.REFRESH_TOKEN_SECRET' });
            if(await this.tokenRepository.findOne({ where: { userID: user.id, token }})){
                request.user = user;
                return true;
            }
            throw new Error
